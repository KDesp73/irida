...
last_command       : [] u8,
uci_options        : std.StringHashMap(Option),
...
pub fn init(alloc: std.mem.Allocator) !Uci {
    var self = Uci{
        ...
        .depth_limit  = 4,
        .max_bookmoves = 10,
        .uci_options  = std.StringHashMap(Option).init(alloc),
        .last_command = "",
    };

    self.searcher = try Searcher.init(alloc);
...
fn populateOptions(self: *Uci) !void {
    try self.uci_options.put("Hash", Option {
        .name = "Hash",
        .type = .spin,
        .value = .{ .spin = 64 },
    ...
    try self.uci_options.put("Ponder", Option {
        .name = "Ponder",
        .type = .check,
        .value = .{ .check = false },
        .params = .none,
    ...
    try self.uci_options.put("SkillLevel", Option {
        .name = "SkillLevel",
        .type = .spin,
        .value = .{ .spin = 20 },
    ...
    try self.uci_options.put("SyzygyPath", Option {
        .name = "SyzygyPath",
        .type = .string,
        .value = .{ .string = "" },
        .params = .none,
    ...
    try self.uci_options.put("SyzygyProbeDepth", Option {
    ...
    try self.uci_options.put("Syzygy50MoveRule", Option {
    ...
    try self.uci_options.put("SyzygyProbeLimit", Option {
...
fn uci(self: *Uci) !void {
    ...
    log.id("name", Engine.name);
    log.id("author", Engine.author);
