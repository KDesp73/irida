...
void LoadUciConfig(UciState* state)
{
    ...
    state->uciOptions[state->uciOptionCount++] = (UciOption) {
        .name = "EvalFile",
        .type = UCI_STRING,
        .value.string = "nn/nn-37f18f62d772.nnue",
        .default_value = "nn/nn-37f18f62d772.nnue"
    };
...
void PrintUciOptions(UciState* state)
{
    ...
    for(size_t i = 0; i < state->uciOptionCount; ++i) {
        UciOption option = options[i];

        printf("option name %s type %s", option.name, UciOptionTypeString[option.type]);
        if(strlen((option.default_value))) printf(" default %s", option.default_value);
        printf("\n");
    }
}
