...
Move irida_Search(Board* board, EvalFn eval, OrderFn order, SearchConfig* config)
{
    ...
    if (config->useSyzygy && piece_count <= config->syzygyProbeLimit &&
        ...
        if (irida_SyzygyProbeRoot(board, config->syzygy50MoveRule, &tb_move)) {
    ...
    irida_SearchStartTimer(config->timeLimitMs);
                   ...
                   SearchConfig* config)
{
    ...
    int tt_score = 0;
    if (config->useTT && irida_TTProbe(board->hash, depth, alpha, beta, ply, &tt_score, &tt_move))
        return tt_score;
    ...
    if (tb_wdl_eligible) {
        int tb_score = irida_SyzygyProbeWdl(board, config->syzygy50MoveRule);
        if (tb_score != SYZYGY_PROBE_FAILED) {
            if (config->useTT)
                irida_TTStore(board->hash, depth, tb_score, TT_EXACT, NULL_MOVE, ply);
    ...
    if (config->useTT) {
        ...
        irida_TTStore(board->hash, depth, bestScore, type, bestMove, ply);
    }
