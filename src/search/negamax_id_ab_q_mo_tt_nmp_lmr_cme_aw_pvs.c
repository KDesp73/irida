    ...
    for (int currentDepth = 1; currentDepth <= config->maxDepth; currentDepth++) {
        ...
        for (;;) {
            ...
            for (size_t i = 0; i < legal.count; i++) {
                ...
                int score;
                if (!config->usePVS || root_ord == 0 || alpha + 1 >= beta) {
                    score = -negamax_rec(board, eval, order, currentDepth - 1, 1, -beta, -alpha, config);
                } else {
                    score = -negamax_rec(board, eval, order, currentDepth - 1, 1, -alpha - 1, -alpha, config);
                       ...
                       SearchConfig* config)
{
    ...
    for (size_t i = 0; i < legal.count; i++) {
        ...
        const bool pv_move = (move_index == 0);
        int c_alpha = -beta;
        int c_beta = -alpha;
        const bool try_zw = config->usePVS && !pv_move && (alpha + 1 < beta);
        if (try_zw) {
            c_alpha = -alpha - 1;
            c_beta = -alpha;
        }
        ...
        if (use_lmr) {
            ...
            score = -negamax_rec(board, eval, order, reduced_depth, ply + 1, c_alpha, c_beta, config);

            const bool need_more = (reduced_depth < next_depth && (score > alpha || score >= beta))
                || (try_zw && score > alpha && score < beta);

            if (need_more) {
                score = -negamax_rec(board, eval, order, next_depth, ply + 1, -beta, -alpha, config);
            }
        } else if (try_zw) {
            score = -negamax_rec(board, eval, order, next_depth, ply + 1, c_alpha, c_beta, config);
            if (score > alpha && score < beta) {
                score = -negamax_rec(board, eval, order, next_depth, ply + 1, -beta, -alpha, config);
            }
        } else {
